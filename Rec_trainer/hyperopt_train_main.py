...
from fea_config import sparse_features,dense_features,task
...
def makedata():
    ...
    """
    data = pd.read_csv('/media/liang/Nas/推荐系统数据集/criteo_sampled_data.csv')#.sample(frac=0.1, replace=False, random_state=5, axis=0)
...
#    config['fea_model']['sparse_fea_path'] = './featuremodel/{}_encoder.pkl'.format('sparse_fea')
#    config['fea_model']['dense_fea_path'] = './featuremodel/{}_encoder.pkl'.format('dense_mms')
    ...
    output = open(config['fea_model_path'], 'wb')
